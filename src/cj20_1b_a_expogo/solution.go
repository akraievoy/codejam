...
const Impossible = "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"

func probe(jm Jam, du, dd, dl, dr, prefix string, x, y int64) string {
	if x == 0 && y == 0 {
		jm.Debug("x=", x, "y=", y, " --> ", prefix)
		return prefix
	}
	if (x+y)%2 == 0 {
		jm.Debug("x=", x, "y=", y, " --> ", Impossible)
		return Impossible
	}
	if x%2 == 1 {
		probeL := Impossible
		if y != 0 || x != 1 {
			probeL = probe(jm, du, dd, dl, dr, prefix+dl, (x+1)/2, y/2)
		}
		probeR := Impossible
		if y != 0 || x != -1 {
			probeR = probe(jm, du, dd, dl, dr, prefix+dr, (x-1)/2, y/2)
		}
		route := TernS(len(probeL) < len(probeR), probeL, probeR)
		jm.Debug("x=", x, "y=", y, " --> ", route)
		return route
	} else if y%2 == 1 {
		probeD := Impossible
		if x != 0 || y != 1 {
			probeD = probe(jm, du, dd, dl, dr, prefix+dd, x/2, (y+1)/2)
		}
		probeU := Impossible
		if x != 0 || y != -1 {
			probeU = probe(jm, du, dd, dl, dr, prefix+du, x/2, (y-1)/2)
		}
		route := TernS(len(probeD) < len(probeU), probeD, probeU)
		jm.Debug("x=", x, "y=", y, " --> ", route)
		return route
	} else {
		panic("HOUSTON 1")
...
func solveAll(jm Jam) {
	...
	for t := int64(1); t <= T; t++ {
		...
		route := probe(jm, du, dd, dl, dr, "", x, y)
		if route == Impossible {
