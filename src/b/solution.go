...
func solveAll(jm Jam) {
	...
	for t := int64(1); t <= T; t++ {
		df("test %d of %d\n", t, T)
		c, d := i8(), i8()
		x := make([]NodeVisit, c, c)
		updatedAt := make([]int64, c, c)
		for cc := int64(1); cc < c; cc++ {
			x[cc] = NodeVisit{cc, -i8()}
			updatedAt[cc] = x[cc].SeenAfter
			if x[cc].SeenAfter < 0 {
				panic("the other test set not supported")
			}
		}
		sort.Sort(SortNodeVisit(x))

		links := make([]*DirectLink, 0, d)
		linksByNode := make([][]*DirectLink, c, c)
		for i := int64(0); i < c; i++ {
			linksByNode[i] = make([]*DirectLink, 0, c)
		}
		for dd := int64(0); dd < d; dd++ {
			l := DirectLink{i8() - 1, i8() - 1, 0}
			links = append(links, &l)
			linksByNode[l.Source] = append(linksByNode[l.Source], &l)
			linksByNode[l.Target] = append(linksByNode[l.Target], &l)
		}

		for _, nv := range x {
			if nv.SeenAfter == 0 {
				continue // that's the source
			}
			for _, l := range linksByNode[nv.NodeIndex] {
				otherNode := TernI8(l.Source == nv.NodeIndex, l.Target, l.Source)
				l.Latency = MaxI8(1, AbsI8(updatedAt[otherNode] - updatedAt[nv.NodeIndex]))
			}
		}
...
type SortNodeVisit []NodeVisit

func (s SortNodeVisit) Len() int           { return len(s) }
func (s SortNodeVisit) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
func (s SortNodeVisit) Less(i, j int) bool { return s[i].SeenAfter < s[j].SeenAfter }
