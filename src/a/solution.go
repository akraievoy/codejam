...
func solveAll(jm Jam) {
	...
	for t := int64(1); t <= T; t++ {
		df("test %d of %d\n", t, T)
		l, r, i, swap := i8(), i8(), int64(1), false
		int64OverflowSanity := big.NewInt(2 * (l + r))
		for l >= i || r >= i {
			if l < r {
				l, r, swap = r, l, !swap
			}
			if l == r {
				swap = false //	take from left if equal
			}
			// l >= r from now on
			if l-r >= i {
				takenA := int64(sort.Search(
					int(l/i+1),
					func(ta int) bool {
						return l-sum(i, int64(ta), int64OverflowSanity) < r
					},
				))
				l -= sum(i, takenA, int64OverflowSanity)
				i += takenA
				if l+i-1 == r {
					swap = false
				}
				...
				takenB := int64(sort.Search(
					int((l+r)/i+1),
					func(ta int) bool {

						return l-sum2(i, int64((ta+1)/2), int64OverflowSanity) < 0 || r-sum2(i+1, int64(ta/2), int64OverflowSanity) < 0
					},
				))
				l -= sum2(i, int64((takenB+1)/2), int64OverflowSanity)
				r -= sum2(i+1, int64(takenB/2), int64OverflowSanity)
				i += takenB
				if takenB%2 == 0 {
					r += i - 1
					i--
		...
		pf("Case #%d: %d %d %d\n", t, i-1, TernI8(swap, r, l), TernI8(swap, l, r))
	}
...
func sum(from int64, count int64, sanity *big.Int) int64 {
	if _i().Rsh(_i().Mul(_ii(count), _ii(from+from+count-1)), 1).Cmp(sanity) >= 0 {
		return sanity.Int64()
	}
	return count * (from + from + count - 1) / 2
}

func _i() *big.Int {
	return big.NewInt(0)
}
func _ii(i int64) *big.Int {
	return big.NewInt(i)
}

func sum2(from, count int64, sanity *big.Int) int64 {
	if _i().Rsh(_i().Mul(_ii(count), _ii(from+from+(count-1)*2)), 1).Cmp(sanity) >= 0 {
		return sanity.Int64()
	}
	return count * (from + from + (count-1)*2) / 2
}
